...
merged <- as_tibble(read.csv('TRAITS_W_MARKERS.csv', stringsAsFactors = F))
merged$X <- NULL

# Create list of marker names
markers_used <- tibble(MARKER_NM = unique(merged$MARKER_NM))
```

```{r}
trait_marker <- split(merged, f = list(merged$COOP_LOC_NM, merged$TRAIT_NM, merged$MARKER_NM))
trait_marker <- trait_marker[sapply(trait_marker, function(x) dim(x)[1] > 0)]
```

```{r}
trait_marker_names <- names(trait_marker)
tm_lodging <-  trait_marker[grep("^.*LODGING", trait_marker_names)]

tm_non_lodging <- trait_marker[grep("^(?!.*LODGING)", trait_marker_names, perl = T)]
...
```{r}
p_values <- list()
for (i in seq_along(tm_non_lodging)) {
  line_AA = filter(tm_non_lodging[[i]], HAP_CALL == 'AA')
  line_BB = filter(tm_non_lodging[[i]], HAP_CALL == 'BB')
  if (nrow(line_AA) <= 1) next
  if (nrow(line_BB) <= 1) next
  test = t.test(line_AA$norm_obs, line_BB$norm_obs)
  test_df = tibble(MARKER_NM = tm_non_lodging[[i]]$MARKER_NM[1], TRAIT_NM = tm_non_lodging[[i]]$TRAIT_NM[1], COOP_LOC_NM = tm_non_lodging[[i]]$COOP_LOC_NM[1], p.value = test[3])
  p_values[[i]] <- test_df
}

all_p_values <- bind_rows(p_values)
all_p_values$p.value <- as.numeric(unlist(all_p_values$p.value))
```

```{r}
# find which p-values were significant. I used 0.05 here.
significant <- filter(all_p_values, p.value <= 0.05)
```

```{r}
p_values <- list()
for (i in seq_along(tm_lodging)) {
  line_AA = filter(tm_lodging[[i]], HAP_CALL == 'AA')
  line_BB = filter(tm_lodging[[i]], HAP_CALL == 'BB')
  if (nrow(line_AA) <= 1) next
  if (nrow(line_BB) <= 1) next
  test = coin::wilcox_test(line_AA$OBSRV_NT_NBR ~ line_BB$OBSRV_NT_NBR)
  test_df = tibble(MaRKER_NM = tm_lodging[[i]]$MARKER_NM[1], TRAIT_NM = tm_lodging[[i]]$TRAIT_NM[1], COOP_LOC_NM = tm_lodging[[i]]$COOP_LOC_NM[1], p.value = test[3], diff.location = test[9][[1]])
}
```
